...
# modified: Sophie Stuhler, June 20, 2018
...
generic_ews <- function(timeseries, winsize = 50, detrending = c("no", "gaussian", 
                        ...
                        logtransform = FALSE, interpolate = FALSE, AR_n = FALSE, powerspectrum = FALSE, do_plot = FALSE) {
  ...
  # Interpolation
  if (interpolate) {
    posNaN <- which(is.na(Y))
    YY <- approx(timeindex, Y, n = length(Y), method = "linear")
    Y[posNaN] <- YY$y[posNaN]
  } else {
    Y <- Y
  }
